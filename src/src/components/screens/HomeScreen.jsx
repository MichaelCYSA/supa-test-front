...
export default function HomeScreen() {
  const { user, energyStatus, adStatus, levelRecords, startLevel, watchAd, setScreen, notify } = useStore();
    ...
    <div style={{ padding: '16px 16px 100px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: '#555', marginTop: 1 }}>
            💥 {(user.total_balls_destroyed || 0).toLocaleString()} balls destroyed
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ color: '#a855f7', fontWeight: 700, fontSize: 14, marginTop: 2 }}>
            💎 {user.gems}
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: info.tierColor, fontSize: 10, fontWeight: 700,
          padding: '3px 10px', borderRadius: 20, letterSpacing: 1,
        }}>
          {info.tierName.toUpperCase()}
        </div>

        <div style={{ fontSize: 11, color: '#555', letterSpacing: 2, marginBottom: 4 }}>CURRENT LEVEL</div>
        <div style={{ fontFamily: "'Exo 2', sans-serif", fontWeight: 900, fontSize: 52, color: '#fff', lineHeight: 1 }}>
          {level}
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <span style={{ fontWeight: 700, fontSize: 14, color: '#67e8f9' }}>⚡ Energy</span>
        ...
        <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginBottom: 10 }}>
          {Array.from({ length: energyStatus.max }).map((_, i) => (
            <div key={i} style={{
              ...
              background: i < energyStatus.current ? 'linear-gradient(135deg, #06b6d4, #0891b2)' : 'rgba(6,182,212,0.1)',
              border: `2px solid ${i < energyStatus.current ? '#06b6d4' : 'rgba(6,182,212,0.2)'}`,
              boxShadow: i < energyStatus.current ? '0 0 10px rgba(6,182,212,0.4)' : 'none',
              transition: 'all 0.3s',
            }} />
          ))}
        </div>

        {energyStatus.current < energyStatus.max && energyStatus.secondsUntilNext > 0 && (
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {/* Watch ad */}
          <button
            onClick={() => canWatchAd ? useStore.getState().watchAd() : notify('No more ads today', 'error')}
            style={{
              flex: 1, padding: '9px 6px', borderRadius: 10, border: 'none', cursor: 'pointer',
              background: canWatchAd ? 'rgba(6,182,212,0.15)' : 'rgba(40,40,60,0.5)',
              color: canWatchAd ? '#67e8f9' : '#444',
              fontWeight: 600, fontSize: 11,
              border: `1px solid ${canWatchAd ? 'rgba(6,182,212,0.3)' : 'rgba(60,60,80,0.3)'}`,
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              color: '#c084fc', fontWeight: 600, fontSize: 11, cursor: 'pointer',
            }}
          >
            💎 Buy Energy
          </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {[
          { label: '⚔️ Upgrades', sub: 'Upgrade your gun', screen: 'upgrades', color: '#f97316' },
          { label: '🛒 Shop', sub: 'Buy gems & coins', screen: 'shop', color: '#a855f7' },
          { label: '📋 Quests', sub: 'Earn free rewards', screen: 'tasks', color: '#22c55e' },
          { label: '🏆 Ranks', sub: 'Global leaderboard', screen: 'leaderboard', color: '#fbbf24' },
        ].map(item => (
          <button key={item.screen} onClick={() => setScreen(item.screen)} style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 15, color: '#e2e8ff' }}>{item.label}</div>
      ...
      {user.login_streak > 0 && (
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: '#555' }}>Come back tomorrow for more rewards</div>
          </div>
        </div>
      )}
    </div>
